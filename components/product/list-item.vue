        ...
        <div class="tp-product-price-wrapper-2">
          <div v-if="item.discount > 0">
            ...
            <span class="tp-product-price-2 old-price">
              Rp {{(Number(item.price) - (Number(item.price) * Number(item.discount)) / 100).toFixed(2)}}
            </span>
          </div>
          <span v-else class="tp-product-price-2 new-price">Rp{{ item.price.toFixed(2) }}</span>
        </div>

        <p>{{ item.description.slice(0, 100) }}</p>
        <div class="tp-product-list-add-to-cart">
          <button @click="cartStore.addCartProduct(item)" class="tp-product-list-add-to-cart-btn">Add To Cart</button>
        </div>
      </div>
    </div>
  </div>
