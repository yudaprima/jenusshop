  ...
  <div class="tp-category-item-4 p-relative z-index-1 text-center">
    ...
    <div class="tp-category-content-4">
      ...
      <div class="tp-category-price-wrapper-4">
        <span class="tp-category-price-4">${{item.price.toFixed(2)}}</span>
        <div class="tp-category-add-to-cart">
          <button v-if="!cartStore.cart_products.some((prd) => prd.id === item.id)" @click="cartStore.addCartProduct(item)" class="tp-category-add-to-cart-4">
            <svg-add-cart-2 />
            Add to Cart
          </button>
          <nuxt-link href="/cart" v-if="cartStore.cart_products.some((prd) => prd.id === item.id)" class="tp-category-add-to-cart-4">
            <svg-add-cart-2 />
            View cart
          </nuxt-link>
        </div>
